...
export class Aijs {
    ...
    static deepThinkAllAB(map: MapArray, turn_player: number, depth: number, a: number | undefined, b: number | undefined, nearwin: boolean, evalparam: number[][]): [Hand | undefined, number] {
        ...
        const nodeList: HandNode[] = Rule.getNodeMap(map, turn_player);
        for (let i = 0; i < nodeList.length; i++) {
            const hand: Hand = nodeList[i][0];
            const map0 = nodeList[i][1];
            let sc = 0;
            // 必勝            
            const end: number = Rule.isEndX(map0, nearwin);
            if (end === turn_player) {
